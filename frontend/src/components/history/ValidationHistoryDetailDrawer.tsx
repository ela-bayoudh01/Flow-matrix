...
interface ValidationHistoryDetailDrawerProps {
  entry: ValidationHistoryOut | null;
  onClose: () => void;
}
      ...
      <Box sx={{ width: { xs: "100vw", md: 480 }, p: 2 }}>
        ...
        <Table size="small">
          <TableBody>
            ...
            <DetailRow label="Changement" value={`${entry.old_status ?? "—"} → ${entry.new_status}`} />
            <DetailRow label="Décidé par" value={entry.validated_by ?? "—"} />
          </TableBody>
        </Table>
        ...
        {isRuleChange ? (
          <>
            ...
            <Table size="small" sx={{ mb: 2 }}>
              <TableBody>
                <DetailRow label="Action observée avant" value={entry.observed_action_before ?? "—"} />
                <DetailRow label="Action décidée" value={entry.decided_action ?? "—"} />
              </TableBody>
            </Table>
            <Typography variant="subtitle2" sx={{ mb: 1 }}>
              Justification
          ...
          <Alert severity="info">
            ...
            bouton "Changer la règle" depuis la Table des flux ou le Cycle de validation.
          </Alert>
        )}
      </Box>
    </Drawer>
